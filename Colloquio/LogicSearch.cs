...
namespace Colloquio
{
    public static class LogicSearch         //logic and algorithms for searching
    {
        ...
        public static void DbConnect()        //connection to database
        {
            try
            {
                conn = new SqlConnection(@"Data Source=(LocalDB)\MSSQLLocalDB;AttachDbFilename=C:\Users\white\source\repos\Colloquio\Colloquio\dbColloquio.mdf;Integrated Security=True;Connect Timeout=30");
                conn.Open();
            }
        ...
        public static void SearchByIndexes(ListBox listBoxAmbulatorio, ListBox listBoxParteCorpo, int indexAmbulatorio, int indexParteCorpo, ListView listViewEsami)      //selecting ambulatori and partiCorpo new search
        {
            try
            {
                string queryEsame = "SELECT E.[idEsame], E.[nomeEsame], E.[codiceMinisteriale], E.[codiceInterno], E.[descrizione] FROM[ambulatori] AS A INNER JOIN[partiCorpo] AS P ON A.[idParteCorpo] = P.[idParteCorpo] INNER JOIN[esami] AS E ON P.[idEsame] = E.[idEsame] WHERE A.[idAmbulatorio] = " + indexAmbulatorio + " AND P.[idParteCorpo] = " + indexParteCorpo + ";";
                string valueFetchedEsame = ExecuteQueryExam(queryEsame, listViewEsami);
            }
            catch (Exception)
            {
            ...
            public static void SearchByBox(string valueToSearch, int indexComboBox, ListView listViewEsami, ListBox listBoxAmbulatori, ListBox listBoxPartiCorpo)       // seach exam on database
        {
            ...
            try
            {
                DbConnect();// "+ typeOfSearch + "] LIKE '%" + valueToSearch + "%'";
                string columnOfSearch = "";
                ...
                {
                    string queryEsame = "SELECT E.[idEsame], E.[nomeEsame], E.[codiceMinisteriale], E.[codiceInterno], E.[descrizione] FROM[ambulatori] AS A INNER JOIN[partiCorpo] AS P ON A.[idParteCorpo] = P.[idParteCorpo] INNER JOIN[esami] AS E ON P.[idEsame] = E.[idEsame] WHERE E.[nomeEsame] LIKE '%" + valueToSearch + "%' OR E.[codiceMinisteriale] LIKE '%" + valueToSearch + "%' OR  E.[codiceInterno] LIKE '%" + valueToSearch + "%' OR E.[descrizione] LIKE '%" + valueToSearch + "%'";
                    queryAmbulatorio = "SELECT A.[nomeAmbulatorio] FROM[ambulatori] AS A INNER JOIN[partiCorpo] AS P ON A.[idParteCorpo] = P.[idParteCorpo] INNER JOIN[esami] AS E ON P.[idEsame] = E.[idEsame] WHERE E.[nomeEsame] LIKE '%" + valueToSearch + "%' OR E.[codiceMinisteriale] LIKE '%" + valueToSearch + "%' OR  E.[codiceInterno] LIKE '%" + valueToSearch + "%' OR E.[descrizione] LIKE '%" + valueToSearch + "%'";
                    queryParteCorpo = "SELECT P.[parteCorpo] FROM[partiCorpo] AS P INNER JOIN[ambulatori] AS A ON A.[idParteCorpo] = P.[idParteCorpo] INNER JOIN[esami] AS E ON P.[idEsame] = E.[idEsame] WHERE E.[nomeEsame] LIKE '%" + valueToSearch + "%' OR E.[codiceMinisteriale] LIKE '%" + valueToSearch + "%' OR  E.[codiceInterno] LIKE '%" + valueToSearch + "%' OR E.[descrizione] LIKE '%" + valueToSearch + "%'";
                    valueFetchedEsame = ExecuteQueryExam(queryEsame, listViewEsami);
                }
                else           //if selected anything else
                {
                    string queryEsame = "SELECT E.[idEsame], E.[nomeEsame], E.[codiceMinisteriale], E.[codiceInterno], E.[descrizione] FROM[ambulatori] AS A INNER JOIN[partiCorpo] AS P ON A.[idParteCorpo] = P.[idParteCorpo] INNER JOIN[esami] AS E ON P.[idEsame] = E.[idEsame] WHERE E.[" + columnOfSearch + "] LIKE '%" + valueToSearch + "%'";
                    queryAmbulatorio = "SELECT A.[nomeAmbulatorio] FROM[ambulatori] AS A INNER JOIN[partiCorpo] AS P ON A.[idParteCorpo] = P.[idParteCorpo] INNER JOIN[esami] AS E ON P.[idEsame] = E.[idEsame] WHERE E.[" + columnOfSearch + "] LIKE '%" + valueToSearch + "%'";
                    queryParteCorpo = "SELECT P.[parteCorpo] FROM[partiCorpo] AS P INNER JOIN[ambulatori] AS A ON A.[idParteCorpo] = P.[idParteCorpo] INNER JOIN[esami] AS E ON P.[idEsame] = E.[idEsame] WHERE E.[" + columnOfSearch + "] LIKE '%" + valueToSearch + "%'";
                    valueFetchedEsame = ExecuteQueryExam(queryEsame, listViewEsami);
                }
